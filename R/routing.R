...
#' observations) of the VIC model (use `Lohmann_UH`);
...
#' `Lohmann_conv()`).
#'
#' @param file_dir Flow direction raster file of the basin (should be
...
#' [Wu et al. (2012)](http://www.ntsg.umt.edu/project/drt.php).
#'
...
#' corresponding to `file_dir`. If not provided, those fractions would
...
#' to `file_dir`, or a single value for all gridcells. Default 800.
#'
...
#'
#' @param uh Output of the function `Lohmann_UH` that containing the unit
...
#' of river channel networks of the basin (parameter `direc_file`),
#' coordinates of the gridcells of the VIC model (`soil_param`), location
#' of the pour point (parameter `stn_x` and `stn_y`, usually is the
...
#' `uh_box`). This uses the function `Lohmann_UH()`:
#'
#' \preformatted{
...
#' }
#'
#' The fraction file (`fract`) determines the fraction of the gridcells
...
#' }
#'
#' `runoff_table` should be a output table of function [vic()]
#' that containing two variables: `OUT_RUNOFF` and `OUT_BASEFLOW`.
#' Thus the parameter `output_info` of [vic()] can be set as:
#'
#' \preformatted{
#' out_info <- list(
#'   runoff_table = list(
...
#' @export
Lohmann_UH <- function(file_dir, soil_params, stn_x, stn_y, fract = NULL,
